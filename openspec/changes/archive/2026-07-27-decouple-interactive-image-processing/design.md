...
平滑处理必须使用与逐像素邻域多数投票等价的线性时间滑动窗口，不得让物理半径以平方复杂度
放大交互代理成本；横向统计和纵向合成都按固定行块检查取消。尺寸提交导致代理缓存键变化时，
前端继续缩放上一张已解码纹理，只有新代理成功可绘制后才原位替换；单个代理失败不得清空其他
图层或该图层上一张可用纹理。
...
20 次连续物理尺寸变化与 UI 可达上限 1 mm 平滑、25 µm 高倍率代理采样、后台重形态学任务期间
拖动、检查器/画布双消费者和清缓存重建。
尺寸 burst 必须证明过时任务能在形态学操作内部取消、最终 generation 被接受且等待期间旧纹理
仍可绘制。运行时暴露仅用于测试与诊断的
...
代理，并以 1 mm 平滑、25 µm 高倍率采样覆盖 UI 可达的最重尺寸 burst；固定输入事件到对应
generation 纹理可绘制的计时边界、至少 30 个非缓存预览样本和至少 300 个拖动帧，并记录应用
commit、macOS 基准机硬件/系统/供电模式和输出尺寸。当前基准机上阈值/
反相首个新代理 p95 不高于 120 ms，拖动帧更新 p95 不高于 16.7 ms，连续尺寸变化的最终重平滑
代理不高于 250 ms；其他平台先记录相同 profile，新增硬门槛需另行校准，不套用未经测量的
绝对数字。
